    ...
    static function bindInternal(source:Expr, target:Expr, mode:BindingMode)
    {
        switch (mode)
        {
            case OneTime:
                switch (source.expr)
                {
                    case EField(vm, prop, _) | EParenthesis(_.expr => ECheckType(_.expr => EField(vm, prop, _), _)):
                        return macro
                        {
            ...
            case OneWay: // setup the IObservable listener
                switch (source.expr)
                {
                    case EField(vm, prop, _) | EParenthesis(_.expr => ECheckType(_.expr => EField(vm, prop, _), _)):
                        return macro
            ...
            case OneWayToSource: // setup the HaxeUI listener
                switch (target.expr)
                {
            ...
            case TwoWay: // setup both listeners
